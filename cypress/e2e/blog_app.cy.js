describe('Blog app', function() {
  ...
  describe('When logged in', function() {
    ...
    it('A blog can be created, liked and removed', function() {
      ...
      cy.get('.inputTitle')
        .type('Testaus, koodaamisen lyijyisin vaihe')
      cy.get('.inputAuthor')
        .type('Testi-Tero')
      cy.get('.inputUrl')
        .type('www.testaansuuvaahdossa.com')
      cy.get('#blogSubmit')
        .click()
      ...
      cy.get('#root')
        .should('not.contain', 'Testaus, koodaamisen lyijyisin vaihe Testi-Tero')
    })
    ...
    it('For more than one blog, blogs are ordered in a descending order by amount of likes', function() {
      ...
      cy.get('.blogContent').eq(0).should('contain', 'The title with the most likes')
      cy.get('.blogContent').eq(1).should('contain', 'The title with the fewest likes')
    })
  })
})